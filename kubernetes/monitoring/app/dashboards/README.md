...
| Services/home-assistant.json | Services | hand-written | — | Home Assistant exposes no Prometheus endpoint (needs `prometheus:` config plus a token minted post-install — tracked follow-up), so request panels come from Beyla, matched on `k8s_namespace_name`; kube-state-metrics series use `exported_namespace`. No network throughput panel: the pod runs `hostNetwork: true`, so `container_network_*` enumerates every interface on the node and would report the node's whole overlay traffic as Home Assistant's — omitted rather than shown misleading. The Beyla panels are also unverified for a host-network pod: the only other host-network workloads here are collectors Beyla deliberately excludes, so they may read empty until Home Assistant actually runs |
