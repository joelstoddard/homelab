...
- **Every tile carries two URLs.** `url` is followed by the browser and is
  ...
  because CoreDNS forwards to the nodes' own resolvers and those are public
  ones that know nothing of the LAN wildcard. Nothing is handed over DHCP:
  the resolvers are set explicitly in the machine config
  (`docs/design/dns-chain.md`). `talosctl get resolverstatus` is the check.
