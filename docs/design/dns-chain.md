...
forwards the rest to the next, until the chain reaches public DNS. The code is
`ansible/roles/dns/`, `ansible/playbooks/dns.yaml`,
`opentofu/resources/pihole/` and `kubernetes/lancache/`.
...
`TODO.md` could not close its "hand out Pi-hole as the ONLY resolver over DHCP"
item — removing the public fallbacks needs a second resolver first. Being
...
missing passthrough gave a valid certificate and a Traefik 404. The cluster
could not resolve the names it serves: the Talos machine config carried no
`nameservers`, so nodes fell through to Talos's built-in `1.1.1.1` and
`8.8.8.8`. And every machine on the LAN pulled the same multi-gigabyte game
update over the same uplink.
...
- **The cluster cannot pull images.** Talos nodes list the Pi-hole VIP and both
  Pi-hole instances and nothing else, deliberately: a public fallback would let
