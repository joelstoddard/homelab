...
> **Validating the talconfig.** The `config` stage's `talhelper genconfig`
> validates the rendered config as it writes the per-node files, so a clean
> config stage is itself the check. To validate the talconfig on its own
> afterwards (note: positional file arg, not a flag):
