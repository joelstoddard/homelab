...
| 6 | `uboot.scr` | `wget /talos/<ver>-<schematic8>/vmlinuz-arm64.raw` (77 MB, flat Image) and `initramfs-arm64.xz` (65 MB, actually zstd) → `booti` with `talos_kernel_cmdline`. The directory is keyed on version *and* schematic — `<schematic8>` is the first 8 hex chars of the Pi schematic ID. | Caddy: the two `GET`s ~2–4 s apart. A repeat `GET /boot/uboot.scr` within a minute means `booti` failed and `reset` fired. |
